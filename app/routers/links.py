...
def create_link(payload: LinkCreate, service: MemoryService = Depends(get_memory_service)) -> LinkResponse:
    return service.create_link(payload)


@router.delete("/memory-links/{link_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_link(link_id: uuid.UUID, service: MemoryService = Depends(get_memory_service)) -> Response:
