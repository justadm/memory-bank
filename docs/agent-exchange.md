...
### 2026-06-11T13:11:53Z - codex-local - memory.bank/SolutionArtifact - import-quality production verification

prod_write: yes

Facts:

- User approved continuing after `d8faefe Harden import quality evidence`.
- `/opt/memlayer` on `msk` was fast-forwarded to `f9652f9`.
- Production API image was rebuilt from `/opt/memlayer/deploy/msk/docker-compose.yml`.
- Only the `api` service was recreated; `db` remained running and healthy.
- `memlayer-api` is running on `127.0.0.1:18120->8000`.
- Local and public health checks returned `{"status":"ok"}` after deploy.
- Controlled reimport ran for `/Users/just/projects/SolutionArtifact` with `--apply --skip-pack --skip-snapshot --smoke`.
- Import used existing project id `be947ae2-c1e3-4cff-be50-65cccb2092e7` and `existing_entry_mode=update`.
- Import result after the fix: `entries_created=0`, `entries_updated=11`, `conflicts_detected=0`, `quality_review_required_count=1`.
- Smoke result: status ok, `search_results_count=5`, `relevant_results_count=5`.
- Read-only check after reimport found one active canonical `Initial project import` event with `import_runs_count=4` and no quality flags.
- Two stale pre-fix active review items remained:
  - `Test suite coverage snapshot` note had stale `quality_review_required` with empty quality flags;
  - `FastAPI service stack` constraint had stale `missing_evidence` and was no longer generated by the fixed importer.
- Minimal direct cleanup was applied with the tenant key:
  - note kept active, `quality_review_required` cleared, `review_status=approved_direct_cleanup`, evidence set to `tests/`;
  - stale FastAPI constraint archived.
- Final read-only check: `active_total=12`, one active canonical import event, `quality_review_required=[]`.

Open work:

- Update SolutionArtifact evidence with deploy/reimport/cleanup results.
- No further production reimport is needed for this specific verification unless new importer changes land.
