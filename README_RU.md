...
Для macOS-проектов в root-pack также появился `memlayer_launchd_install.sh`: он ставит `launchd`-agent и даёт reproducible способ автоматически поднимать MemLayer при логине и затем повторно через настраиваемый интервал, если Docker Desktop стартует позже.
...
Для рискованных действий агентов добавлены машинно-читаемые шаблоны в [docs/revision-evidence-templates.md](/Users/just/apps/memory.bank/docs/revision-evidence-templates.md):
