...
- Added an operational hardening layer around local MemLayer usage: SDK/examples/CLI now prefer `https://memlayer.loc/api`, a new `scripts/runtime_smoke_check.py` verifies `health -> import -> search -> relevant`, and `GET /admin/runtime/self-check` provides a lightweight read-only runtime check after reboots or deploys.
