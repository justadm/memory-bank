...
- Started the public `memlayer.ru` layer in parallel: added a dedicated static-site bundle under `deploy/msk/site` with curated docs content, one-click copy buttons for command blocks, a lightweight `Auto / Light / Dark` theme switcher, and a separate nginx sample for serving `memlayer.ru` as a static root without exposing the private admin host.
- Put that public `memlayer.ru` landing live on `msk` over HTTPS. The root page is now served from the static bundle, and the `/api` docs path resolves through the same static site (currently canonicalized to `/api/`). The remaining ingress work is isolated to `api.memlayer.ru` and `adm.memlayer.ru`, which still wait on recursive DNS propagation before their own certificates can be issued.
- Recursive DNS propagation completed for `api.memlayer.ru` and `adm.memlayer.ru`, after which the final `memlayer.ru` certificate was expanded to include both subdomains. Switched `api.memlayer.ru` and `adm.memlayer.ru` to the final nginx configs from the repo, reloaded nginx successfully, and confirmed live public smoke checks: `https://api.memlayer.ru/health` returns `{"status":"ok"}`, `https://adm.memlayer.ru/api/health` returns `{"status":"ok"}`, and `https://adm.memlayer.ru/` serves the embedded MemLayer console.
- Split the public documentation surface into explicit RU/EN variants. Kept `memlayer.ru/` as the Russian landing, added `memlayer.ru/en/` and `memlayer.ru/en/api/`, and added RU/EN switchers to the static site without exposing `adm.memlayer.ru`.
- Converted the repository documentation layout to `README.md` in English plus `README_RU.md` as the preserved Russian document. Kept the current GitHub repository slug unchanged (`justadm/memory-bank`) so public links stay stable until a later dedicated rename step.
- Hardened the `msk` deployment path for the public admin console by adding deploy-ready nginx Basic Auth assets: a sample snippet under `deploy/msk/nginx/snippets`, a server-side helper script that safely prompts for the password and writes both the htpasswd file and the nginx snippet, and deploy docs for turning this second barrier on for `adm.memlayer.ru`.
- Synced the public `memlayer.ru` favicon with the embedded admin console by copying the same `favicon.svg` asset into the static site bundle and wiring all RU/EN landing and API pages to reference the shared `/favicon.svg`.
- Enabled nginx Basic Auth on the live `adm.memlayer.ru` vhost on `msk`: unauthenticated requests now return `401`, authenticated console access returns `200`, and `api.memlayer.ru/health` remains public and healthy. Fixed the deploy helper to put the htpasswd file in the `www-data` group so nginx can read it while keeping `640` permissions.
- Migrated the local MemLayer knowledge base into the production PostgreSQL contour on `msk`. Took a safety custom-format backup of the previously empty production database at `/opt/memlayer/backups/prod-before-local-restore-20260530.dump`, restored `/opt/memlayer/backups/local-knowledge-20260530.dump`, and verified production counts now match local: 9 projects, 513 memory entries, and 54 memory links. Public API health and Basic Auth-protected admin health both remained healthy after restore.
- Switched the project root-pack defaults from localhost-first to production-first MemLayer. New packs now prefer `https://api.memlayer.ru`, keep local endpoints as fallback, preserve existing secrets and custom URLs, and migrate only old managed defaults such as `MEMLAYER_API_URL=http://127.0.0.1:18100`, the old extra URL ladder, and `MEMLAYER_RETRY_ATTEMPTS=10`. Reinstalled the root-pack for 41 top-level local projects plus `/Users/just/projects/ofs/lk.loc`, seeding the production API key from `msk` into ignored `.memlayer/.env.memlayer` files without printing it.
